	...
	constructor(context: vscode.ExtensionContext) {
		...
		const disposableCompletionProvider = vscode.languages.registerInlineCompletionItemProvider('*', {
			provideInlineCompletionItems: async (document, position, context, cancellationToken) => {
				...
				const autoCompletionModel: CompletionModel = config.get('autoCompletionModel') || "text-davinci-002";
				const numberOfSuggestions: number = config.get('numberOfCodeSuggestions') || 1;
				...
				const suggestions = this.cache[prompt] || await getCodeCompletions({
					prompt,
					cancellationToken,
